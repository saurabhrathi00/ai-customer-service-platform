...
public class EnableXMediaStreamHandler implements TelephonyMediaStreamHandler {

    private static final Logger log = LoggerFactory.getLogger(EnableXMediaStreamHandler.class);
    ...
    public boolean validateHandshake(ServerHttpRequest request) {
        log.info("[enablex] validating handshake uri={}", request.getURI());
        return true;
    }

    @Override
    public void onConnect(CallSession session, Map<String, String> connectParams) {
        log.info("[enablex] onConnect callId={}", session.getCallId());
        ...
        if (ws != null) {
            ...
            log.info("[enablex] onConnect query from handshake attrs: {}", query);
            if (query != null) {
                ...
                log.info("[enablex] session hydrated businessId={} businessName={} customerPhone={}",
    ...
    public void onInboundFrame(CallSession session, String payload) {
        try {
            ...
            switch (event) {
                ...
                default -> log.warn("[enablex] <- UNKNOWN event '{}' callId={}", event, session.getCallId());
            }
        } catch (Exception ex) {
            log.error("[enablex] frame parse error callId={} payload={}",
    ...
    private void handleStart(CallSession session, JsonNode root) {
        ...
        session.getProviderAttributes().put("enablexCallSid", callSid);

        AudioCodec codec = AudioCodec.MULAW;
        if (encoding.contains("l16") || encoding.contains("pcm")) {
            codec = AudioCodec.PCM16;
        }
        session.getProviderAttributes().put("codec", codec);

        log.info("[enablex] call started callId={} encoding={} sampleRate={} codec={}",
        ...
        if (!cp.isMissingNode()) {
            ...
            log.info("[enablex] session hydrated from start frame businessId={} customerPhone={}",
        ...
        try {
            ...
            log.error("[enablex] failed to open AI conversation WS callId={}", session.getCallId(), ex);
            return;
        }
    ...
    private void safeEndAiConversation(String callId) {
        try {
            ...
            log.warn("[enablex] onCallEnd failed callId={}: {}", callId, ex.getMessage());
        }
    }
        ...
        public void onHangup(String callId, String spokenText, String reason) {
            ...
            ttsTail.updateAndGet(prev -> prev.thenRunAsync(() -> {
                try {
                    ...
                    log.warn("[enablex] hangup TTS failed callId={}: {}", callId, ex.getMessage());
                }
                closeStream(callId, ws);
            }, ttsExecutor).exceptionally(ex -> { closeStream(callId, (WebSocketSession) session.getProviderAttributes().get("ws")); return null; }));
        }
